...
export function processDataForModel(
  ...
  const targetValuesSet = new Set(data.map((row: any) => row[targetVariable]));
  ...
  if (isClassification) {
    ...
    const positiveClass = sortedTargets[sortedTargets.length - 1];
    y = data.map((row: any) => (row[targetVariable] === positiveClass ? 1 : 0));
  } else {
