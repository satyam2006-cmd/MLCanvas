  ...
  const preprocessData = useCallback(async (data: CsvData) => {
    ...
    try {
      ...
      if (!processedData || !processedData.length) {
        console.error('[Preprocessing] No data returned after preprocessing. Input data:', {
          inputDataLength: data.length,
          selectedColumns: Object.entries(selectedColumns).filter(([_, selected]) => selected).map(([col]) => col),
          targetColumn,
          firstFewRows: data.slice(0, 2)
        });
