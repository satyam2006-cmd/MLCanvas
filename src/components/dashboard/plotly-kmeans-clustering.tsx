...
interface Trace extends Partial<PlotData> {
  x: number[];
  y: number[];
  mode: 'markers' | 'lines' | 'lines+markers' | 'text' | 'text+markers' | 'text+lines' | 'text+lines+markers' | 'none';
  type: 'scatter';
  name: string;
  marker: {
    color: string;
...
export function PlotlyKmeansClustering() {
  ...
  const traces: Trace[] = Array.from({ length: numClusters }, (_, i) => {
  ...
  const layout: Partial<Layout> = {
    title: {
      text: `K-Means Clustering (K=${numClusters}) on PCA Projection`,
      font: {
        size: 16,
        family: 'Arial',
      },
