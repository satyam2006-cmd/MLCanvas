  ...
  const handleTrainAll = async () => {
    ...
    for (const modelType of MODELS_TO_COMPARE) {
      ...
      if (result.success && result.accuracy) {
        ...
        let metricValue = result.accuracy;
        let lossName = "MSE";
        let lossValue = result.mse || 0;
        
        if (modelType === 'linear' && result.r2 !== undefined) {
            metricName = "R-Squared";
            metricValue = result.r2;
            lossName = "MSE";
            lossValue = result.mse || 0;
        }
        if (['knn', 'dt'].includes(modelType)) {
            metricName = "Accuracy";
            metricValue = result.accuracy;
            lossName = "Classification Error";
            // Calculate classification error as 1 - accuracy if not provided
            lossValue = 1 - result.accuracy;
        }
